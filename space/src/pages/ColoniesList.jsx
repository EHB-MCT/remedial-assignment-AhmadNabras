...
const ColoniesList = () => {
  ...
  const fetchColonies = () => {
    getColonies()
      .then((res) => {
        ...
        // ✅ keep per-colony history
        setAnalyticsData((prev) => {
          const newData = { ...prev };
          data.forEach((c) => {
            if (c.dead) {
              // dead colonies freeze → keep history but don’t add new points
            ...
            if (!newData[c._id]) newData[c._id] = [];
            newData[c._id] = [...newData[c._id], entry].slice(-20); // keep last 20 points
          });
          return newData;
        });
  ...
  useEffect(() => {
    fetchColonies();
    const interval = setInterval(fetchColonies, 10000); // refresh every 10s
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.analyticsSection}>
        ...
        >
          This chart shows the evolution of water, oxygen, energy, and
          production storage for your colonies. Select a colony and a metric to
          focus, or view them all together. Data updates every 10 seconds.
        </p>

        <div style={{ marginBottom: "15px" }}>
          <label style={{ color: "white" }}>Select Colony: </label>
          <select
        ...
        <ResponsiveContainer width="80%" height={400}>
          <LineChart data={chartData}>
            ...
            {selectedColony === "all"
              ? Object.keys(analyticsData).map((id, idx) => {
